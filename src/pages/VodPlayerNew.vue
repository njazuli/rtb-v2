      ...
      >
        <div class="col-12 col-md-12 col-lg-7">
          ...
          <div style="width:100%" class="mb-4 mb-sm-0">
            <p v-if="eps_title" class="eps_player_title">{{ eps_title }}</p>
            <p
              v-if="eps_desc"
              class="eps"
              v-html="eps_desc.replace(/(?:\r\n|\r|\n|\n\n)/g, '<br />')"
            ></p>
            <div style="width:100%;" class="pt-3 mb-3 text-left">
...
export default {
  ...
  methods: {
    ...
    playVid(items) {
      this.$router.push(
        'vodplayer?id=' + this.$route.query.id + '&eps_id=' + items.id
      )
      //   this.$route.query.eps_id = items.id;
    ...
    setInitialId() {
      this.$router.push('vodplayer?id=' + this.vod[0].id)
    ...
    setEpsInitialId() {
      ...
      console.log(JSON.stringify(this.current_episodes, null, 2))
      var first_id = this.current_episodes[0].id
      this.$router.push(
        'vodplayer?id=' + this.$route.query.id + '&eps_id=' + first_id
      )
  ...
  computed: {
    filteredItems() {
      return this.vod.filter(item => {
        var checknewsid =
          item.data && item.data['news-episode'] && item.data['news-episode'].id
        var checkmainid = item.data.id
        if (checknewsid) {
          console.log(JSON.stringify(checknewsid, null, 2))
        }
    ...
    filteredVodEpisodes() {
      ...
      if (eps_arr) {
        eps_arr.forEach(item => {
          var checkId = item.data && item.data.program && item.data.program.id

          var checknewsid = item.data && item.data.news && item.data.news.id

          var checkmainid = item.data && item.data.id
          var checkFirstid = item.id
          if (checkId) {
            if (item.data.program.id == this.$route.query.id) {
              val.push(item)
              this.current_episodes.push(item)
              // console.log(item.dateAvailability +" - "+ item.longTitle);
            }
          } else if (checknewsid) {
            ...
            if (item.data.id == this.$route.query.id) {
              val.push(item)
              this.current_episodes.push(item)
              // console.log(item.dateAvailability +" - "+ item.longTitle);
            }
          } else if (checkFirstid) {
            console.log('checlfirstid')
            if (item.id == this.$route.query.id) {
              val.push(item)
              this.current_episodes.push(item)
              console.log(item.dateAvailability + ' - ' + item.longTitle)
