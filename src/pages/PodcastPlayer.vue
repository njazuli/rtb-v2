<template>
  <div class="min-height-for-section mb_controller w-100 pt-livetv">
    ...
    <div class="container-fluid">
      <div
        class="row mt-4 mx-md-3 mb-3 px-md-2 py-4 remove_m_p align-items-center"
      ...
      >
        <div class="col-12 col-md-6">
          ...
          >
            <div
              class="w-100 thumbMaxwidth d-flex justify-content-center flex-column"
            >
              <div class="thumbnailDivWrapper">
                <img
                  :src="eps_img"
              ...
              <div class="playerWrapper">
                <div
                  class="playerWrapperChild embed-responsive embed-responsive-16by9"
                >
                  <div
                    id="vidBox"
      ...
      <div
        ...
        v-if="podcast_episode.length > 0"
      >
        <div class="col-12">
          <p class="mainEpsTitle text-left">More Episodes</p>
        </div>
        <div
          class="col-6 col-md-4 col-lg-3 my-3 cursor-pointer"
          v-for="(items, index) in podcast_episode"
...
import PodcastSuggestion from '../components/podcastPlayer/podcastSuggestion.vue'
...
export default {
  ...
  methods: {
    ...
    generateEpisodeListFor() {
      ...
      this.podcast_episode.push(store)
    },

    filterFolderData(obj) {
      if (obj.data['podcast-episode'] && obj.data['podcast-episode'][0]) {
        this.getFirstContentData(obj.data['podcast-episode'][0].id)
        if (obj.data['podcast-episode']) {
          this.getEpsListContentData(obj.data['podcast-episode'])
        }
      } else {
        this.setFirstIditems(obj)
        if (obj.data.podcast) {
          this.getContentOtherFolderData(obj.data.podcast.id)
        }
      }
    ...
    setFirstIditems(obj) {
      ...
      this.eps_img = this.imgFiltering(obj)
    ...
    async getEpsListContentData(list) {
      ...
      Promise.all(promises).then(function(results) {
        results.forEach(function(response, index) {
          ...
          if (res) {
            vm.podcast_episode.push(response.data.data)
          }

          if (index == results.length - 1) {
            vm.podcast_episode.sort(function(a, b) {
              return b.id - a.id
            })
          }
        })
...
.thumbnailDivWrapper .podcast-img {
  background: 100%;
  height: 100%;
  object-fit: cover;
  overflow: hidden;
