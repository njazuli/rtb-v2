<template>
  <div class="container-fluid p-0 position-relative">
    <div class="w-100 masthead-content">
      <div class="owl-carousel owl-theme" id="podmasthead">
        <div
          class="item"
        ...
        >
          <div class="item_img">
            <img
              v-bind:src="item_podcast.img_link"
              @error="imageLoadError($event)"
            />
          </div>
          <div class="bg-overlay w-100 h-100 d-flex align-items-end">
            <div class="container-fluid p-0 h-200">
              <div class="row h-100 align-items-end px-0 py-4">
                <div class="col-12">
                  <div class="w-100 px-3">
                    <p
                      class="main-title mb-1 mb-md-2"
                      v-if="item_podcast.data.title"
                    >
                      <!-- <Trunquee :text="item_podcast.data.title" :speed="120" /> -->
                      {{ truncateString(item_podcast.data.title, 30) }}
                    </p>
                    <p class="desc mb-2 mb-md-4">
                      <span
                        class="d-none d-md-block"
                        v-if="item_podcast.data.description"
                        >{{
                          truncateString(item_podcast.data.description, 140)
                        }}</span
                      >
                      <span
                        ...
                        v-if="item_podcast.data.description"
                        >{{
                          truncateString(item_podcast.data.description, 80)
                        }}</span
                      >
                    </p>
                  </div>
                ...
                <div class="col-7">
                  <div class="w-100 details-container px-3">
                    <div
                      class="custom-btn"
                      @click="goTo(checkLink(item_podcast))"
                    >
                      <div
                ...
                <div class="col-5 pr-3" style="height:40px;">
                  <div
                    v-if="item_podcast.data.content_type"
                    class="h-100 d-flex justify-content-end align-items-end"
                  >
                    <div class="category-section">
                      <p class="mb-0 category-text">
                        {{ item_podcast.data.content_type[0].title['en_US'] }}
                      </p>
                    </div>
                  </div>
                </div>
  ...
  methods: {
    filterPodFolder() {
      ...
      vm.podcast.forEach(item => {
        console.log('podcast' + JSON.stringify(item.idSchema, null, 2))
      ...
      Vue.nextTick(
        function() {
          vm.installOwlCarouselForMasthead('#podmasthead')
        }.bind(vm)
      )
    },
...
<style>
#podmasthead .owl-item.active.center {
